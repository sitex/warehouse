...
test.describe('Manager Login', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Mock the users endpoint to return a manager for the login page
    await page.route('**/rest/v1/users**', async (route) => {
      if (route.request().url().includes('role=eq.manager')) {
        await route.fulfill({
          status: 200,
  ...
  test('displays shop login title', async ({ page }) => {
    await expect(page.getByRole('heading', { name: /shop login/i })).toBeVisible();
  });

  test('displays welcome message with user name', async ({ page }) => {
    await expect(page.getByText(/welcome/i)).toBeVisible();
    await expect(page.getByText(mockUsers.manager.name)).toBeVisible();
  });
